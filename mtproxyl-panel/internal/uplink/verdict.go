...
type Status struct {
	...
	// писателей и высокая доля ошибок. Оба состояния при живом прокси бывают
	// мгновенными и проходят сами.
	ShortPolls int
	ErrorPolls int
...
func (s *Status) evaluate(threshold float64) {
	...
	if s.ErrorPolls >= sustainedPolls {
		s.Problems = append(s.Problems, fmt.Sprintf(
			"неудачных подключений %.0f%% за интервал (порог %.0f%%), уже %d мин",
			s.FailRate*100, threshold, s.ErrorPolls))
	}
