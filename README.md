...
Второй вариант бота — **сторож**. Он не управляет прокси и намеренно не просит
на это прав: его дело — одно живое сообщение в чате и звонок, когда прокси
перестал работать. Смотрит он в обе стороны сразу: доступность из России (это
вход, вердикт считает сам MTProxyL) и связь прокси с дата-центрами Telegram
(это выход). Второе важно: фильтрация чаще режет именно выход, и тогда зонды
показывают прежние 95%, а клиенты не работают.

```bash
mtproxyl alertbot status                       # Состояние и настройки
mtproxyl alertbot install --token T --chat ID  # Установить
mtproxyl alertbot set alert_threshold 60       # Порог доступности, %
mtproxyl alertbot set connect_fail_threshold 20
mtproxyl alertbot set timezone Asia/Tashkent   # Пусто — определять самому
mtproxyl alertbot start|stop|restart
mtproxyl alertbot logs 100
mtproxyl alertbot update                       # Перекачать бинарник
mtproxyl alertbot uninstall
```

Включённым может быть только один бот: два опросчика в одном чате — это
путаница в том, кто чем управляет. Установка любого из двух останавливает
другого (останавливает, а не сносит: там настройки, заведённые руками), а
выбор хранится в `settings.conf` ключом `TGBOT_VARIANT`. В меню пункт
«Телеграм бот» один и ведёт туда, где вы сейчас находитесь; в панели — тот же
раздел с переключателем вида.

Адресом доставки у сторожа может быть личка, группа или канал. В канале нет
команд и клавиатуры — бот их там не предлагает; чтобы он мог убирать свои
прежние сообщения, ему нужны права администратора.

#### Как ставить

**Всё сразу, одной командой.** Скрипт сам разберётся, что у вас стоит: если
MTProxyL уже установлен — обновит его файлы до форка, **не трогая настройки,
секреты, движок и контейнер**; если не установлен — поставит с нуля и откроет
мастер. Дальше поставит панель со страницей бота и самого сторожа:

```bash
curl -fsSL https://raw.githubusercontent.com/SkyDeaD/MTProxyL/tg-testing/install-fork.sh | sudo sh
```

Токен можно передать сразу: `… | sudo sh -s -- --token 123:AAH… --chat 12345`.
Отказаться от шага: `--no-panel`, `--no-bot`.

После этого появляется всё: пункт «Телеграм бот» в меню MTProxyL с выбором
между ботом-администратором и сторожем, раздел с тем же переключателем в
панели и работающая служба сторожа. Обновления идут из форка — адрес
запоминается в `/opt/mtproxyl/.repo`; вернуться на официальный MTProxyL можно,
удалив этот файл и выполнив `mtproxyl update`.

#### Только сторож, без форка: два пути
...
**обновления автора ему не страшны**: `mtproxyl update` перепишет свои файлы, а
служба, конфиг и бинарник останутся на месте и продолжат работать. Настройки
правятся командами самого бота:

```bash
mtproxyl-alertbot config show
mtproxyl-alertbot config set alert_threshold 70
mtproxyl-alertbot config set timezone Asia/Tashkent
...
берёт файлы оттуда же. Без этого первый же апдейт вернул бы файлы автора и унёс
пункт меню вместе с подкомандой `alertbot` — сам бот при этом продолжил бы
работать, но управлять им пришлось бы командами из пути 1. Обратная сторона:
обновления автора приходят не сразу, а после того, как они слиты в форк.
Вернуться на официальный MTProxyL просто: удалить `/opt/mtproxyl/.repo` и
выполнить `mtproxyl update`.

**Если автор поменяет что-то кардинально.** Сторожу от MTProxyL нужна ровно
одна вещь — команда `mtproxyl availability status --json`. Пока она отвечает,
бот работает независимо от остальных перемен. Если её формат изменится, бот
скажет об этом строкой в сообщении и записью в журнале
(`journalctl -u mtproxyl-alertbot`), а лечится это обновлением сторожа:
`mtproxyl-alertbot` из свежего релиза либо повторный запуск установщика.
...
переписывает права sudo и перезапускает службу.
