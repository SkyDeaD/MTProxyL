...
установка находит их сама и предлагает набор `awg* wg* tun*`; `std` в меню
ставит его же, `off` убирает. Правила `accept` идут первыми в цепочках
...
`[[upstreams]]`. Самый простой путь, когда движок наш. Подводные камни: только
режим менеджера — конфигом должны владеть мы; через socks уходит **весь**
...
Проще так: свой telemt — берите C; чужая цель реаниматора, посторонний прокси
или tools-only — B, а если разведка не находит живых эндпоинтов (значит wg
режут по сигнатуре) — A.
