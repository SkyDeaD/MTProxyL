...
```bash
mtproxyl geoblock add ir      # Добавить страну в выбранный список
mtproxyl geoblock remove ir   # Удалить страну из списка
mtproxyl geoblock mode whitelist  # Разрешать только выбранные страны
mtproxyl geoblock mode blacklist  # Блокировать выбранные страны
mtproxyl geoblock list        # Список стран и состояние правил
mtproxyl geoblock reapply     # Переприменить вручную
...
В режиме `blacklist` диапазоны выбранных стран блокируются. В режиме
`whitelist` выбранные страны разрешаются, а остальные адреса не доходят до
публичных портов прокси. Режим можно выбрать до добавления стран: при пустом
списке трафик не ограничивается, а правила включаются после добавления первой
разрешённой страны. Loopback (`127.0.0.0/8`) и частные сети RFC1918 (`10/8`,
`172.16/12`, `192.168/16`) в этом режиме разрешены автоматически, поэтому
локальные службы и внутренние адреса не блокируются. Переключение не затрагивает
SSH и другие службы.

Правила живут в `iptables`/`ipset`, а служба `mtproxyl-geoblock.service`
восстанавливает их после перезагрузки. `geoblock list` показывает список,
режим, состояние правил и службы. В раскладке WEB `split` блокировка охватывает
порт обычного прокси и публичный порт WEB. При смене порта, включении или
выключении WEB правила переносятся автоматически. Сохранённые диапазоны входят
в бэкапы и миграцию, поэтому для восстановления не нужен доступ к ipdeny.com.
...
Вместе со встроенным шаблоном разворачиваются `404.html` в том же оформлении,
`robots.txt` и `favicon.ico` — обычный сайт отвечает на эти пути своей
страницей, а не пустотой. Недостающие файлы дописываются при каждом включении
WEB, поэтому после обновления достаточно `mtproxyl web enable`. При выключенной
заглушке (`WEB_DECOY_MODE=empty`) их нет намеренно: этот режим отдаёт пустой
ответ и ничем не выдаёт шаблон. Что именно применено, показывает строка
«Отпечаток» в `mtproxyl web status`. Свои файлы сайта при этом не перезаписываются; если
у сайта нет ни `404.html`, ни `style.css`, MTProxyL кладёт копию главной
страницы, а при выключенной заглушке — пустой файл.
...
`Content-Security-Policy`, `X-Frame-Options` и `Referrer-Policy` заменяются
своими **только у ответов заглушки** — политику bridge-страницы WEB-клиента
менять нельзя, с чужим `script-src` она не запускается. nginx отличает их по
самой политике: если движок разрешает скрипты (`script-src`, `nonce-`,
`unsafe-`), заголовки проходят как есть. Замена: политика движка совпадает с примером из его документации и
опознаётся сканером по одной строке. Набор и порядок директив CSP выводятся из
случайной строки `WEB_FP_SEED`, которая создаётся один раз на установку и
хранится в `settings.conf` — у двух серверов заголовки разные, а при каждом
применении конфига одинаковые. Версия nginx не показывается ни на `:443`, ни в
HTTP-блоке `:80`: `server_tokens off` задан для всего конфига.
...
После запуска telemt MTProxyL проверяет его приватный WEB-listener: `/` и до 64
файлов `.css`/`.js` из каталога сайта должны отвечать `200`. Если snapshot ещё не
содержит файлы, движок один раз перезапускается; повторный `404` отменяет
включение WEB. Статику намеренно не перехватывает nginx через `try_files`:
иначе обычные файлы и carrier-запросы обслуживались бы разными компонентами с
различимым поведением, обходя decoy-политику telemt.
...
Скачанный релиз хранится в `/opt/mtproxyl-zapret2/cache` и при переустановке
берётся оттуда — сеть не нужна. Когда архив на месте, установка спрашивает
«Скачать заново?»: ответ `y` выкачивает релиз с GitHub повторно. Битый архив
удаляется сам, следующая установка скачает его снова.

### Вместе с SYN-лимитером не работает

Zapret2 и NFT SYN-лимитер фильтруют один и тот же трафик, поэтому включён
всегда только один из них. Установка или запуск zapret2 снимает правила
лимитера, убирает его службу и гасит настройку `NFT_ENABLED` — иначе после
перезагрузки лимитер возвращался и работал вместе с zapret2. Включение
Smart-режима лимитера, наоборот, останавливает zapret2. Оставить оба
работающими нельзя: попытка применить правила лимитера при активном zapret2
(и наоборот) завершается отказом с подсказкой, что выключить.
...
| MEKO Runner | Полноэкранная мини-игра с MEKO; в портретном режиме дорожка поднята над зоной для пальца |
...
> Из панели источник выбирается в параметрах WEB Proxy или Selfmask. Путь
> панель видит тот же, что и терминал, но `/tmp` для сайта — плохое место: его
> чистит система. Держите сайт в `/var/www`.
