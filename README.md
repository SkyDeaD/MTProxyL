...
| `--engine-version` | тег релиза telemt, например `3.4.25` | последняя | Версия бинарника. Имеет смысл только с `--engine binary`. |
...
`mtproxyl-block` при старте, поэтому переживают перезагрузку.
