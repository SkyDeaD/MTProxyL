...
# Ветки, с которых в этом репозитории выпускается панель, через пробел. Нужны,
# чтобы отличить «релиза для этой ветки нет — собирай из исходников» от «есть,
# качай готовый». У автора панель выходит с main, у форка тег стоит на
# tg-testing и на main разом — обе ветки релизные. Без этой переменной форк
# тратил минуты на сборку в Docker при готовом бинарнике в своём же релизе.
PANEL_RELEASE_BRANCHES="${MTPROXYL_PANEL_RELEASE_BRANCHES:-tg-testing main}"
