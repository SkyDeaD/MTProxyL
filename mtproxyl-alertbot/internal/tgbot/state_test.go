...
func verdict(pct float64) *mtproxyl.CheckResult {
	return &mtproxyl.CheckResult{
		Percentage:    pct,
		TotalProbes:   20,
...
func failedVerdict() *mtproxyl.CheckResult {
	return &mtproxyl.CheckResult{Error: "сервис проверки не отвечает", Level: mtproxyl.LevelRed}
}
...
func TestDecideIgnoresZeroProbes(t *testing.T) {
	d := Decide(AlertState{}, &mtproxyl.CheckResult{TotalProbes: 0}, DefaultThreshold, time.Now())
